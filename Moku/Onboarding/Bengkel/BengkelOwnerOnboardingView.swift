    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                NavigationLink(destination: BengkelSettingOnboardingView(), isActive: $isNavigateActive) {
                    Button("Lanjutkan", action: { self.isNavigateActive = true})
                        .padding()
            ...
            }.navigationBarTitle("Profil Bengkel")
            
        }
    }
}

struct BengkelSettingOnboardingView: View {
    var body: some View {
        Text("Bengkel settings")
    }
}
