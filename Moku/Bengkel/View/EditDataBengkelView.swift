            ...
            HStack {
                ForEach(0 ..< dayInAWeek.count) { indexDay in
                    Text("\(dayInAWeek[indexDay])")
                        .padding(.all, 7)
                        .background(viewModel.daySelected[indexDay] ? Color(hex: "EB6424") : Color(hex: "D2CFCF"))
                        .cornerRadius(8)
                        .foregroundColor(viewModel.daySelected[indexDay] ? Color.white : Color.gray)
                        .onTapGesture {
                            if viewModel.daySelected[indexDay] {
